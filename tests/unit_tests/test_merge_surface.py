...
social, and search tool groups must keep their expected sizes, stay disjoint,
expose globally-unique tool ``name`` values (a name collision would surface
here), and keep ``_run_meta`` in a single canonical home.
"""
...
def test_tool_names_are_globally_unique() -> None:
    all_tools = [*APIFY_CORE_TOOLS, *APIFY_SOCIAL_TOOLS, *APIFY_SEARCH_TOOLS]
